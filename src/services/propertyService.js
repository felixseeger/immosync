...
import { findPropertyAndReport } from '../utils/checkPropertyImageArtifacts';
...
export const getPropertyTitles = async (propertyIds) => {
    ...
    unique.forEach((id, i) => {
        const d = snaps[i];
        if (d?.exists())
            out[id] = d.data().title ?? id;
    });
    return out;
};
export const addPropertyImage = async (propertyId, imageUrl) => {
    try {
        const propertyRef = doc(db, PROPERTIES_COLLECTION, propertyId);
        await updateDoc(propertyRef, {
...
export const updatePropertyVideos = async (propertyId, videos) => {
    const propertyRef = doc(db, PROPERTIES_COLLECTION, propertyId);
...
export const deletePropertyVideo = async (propertyId, videoUrl) => {
    ...
    const current = (docSnap.data()?.videos ?? []);
...
export const updateProperty = async (propertyId, data) => {
    try {
        const propertyRef = doc(db, PROPERTIES_COLLECTION, propertyId);
        const safe = sanitizeForFirestore(data);
 ...
 */
export const checkLutticherPropertyImageArtifacts = async () => {
    ...
    if (report.summary.likelyArtifacts > 0) {
        console.warn(`Found ${report.summary.likelyArtifacts} image URL(s) that look like thumbnail/cache artifacts.`, report.reports.filter((r) => r.isLikelyThumbOrCache));
    }
    else {

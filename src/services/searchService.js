...
export async function searchFirestoreGrouped(query) {
    const q = query.trim();
    if (!q || q.length < 2) {
    ...
    for (const p of properties) {
        if (results.properties.length >= LIMIT_PER_CATEGORY)
            break;
    ...
    for (const c of contacts) {
        if (results.contacts.length >= LIMIT_PER_CATEGORY)
            break;
    ...
    for (const d of deals) {
        if (results.deals.length >= LIMIT_PER_CATEGORY)
            break;
    ...
    for (const v of viewings) {
        if (results.calendar.length >= LIMIT_PER_CATEGORY)
            break;
        const contactName = contactMap.get(v.contactId)?.name ?? '';
        const propertyTitle = propertyMap.get(v.propertyId)?.title ?? '';
        const dateStr = v.scheduledAt?.toDate?.() ? format(v.scheduledAt.toDate(), 'PPp') : '';
        if (matchesQuery(contactName, q) ||
            matchesQuery(propertyTitle, q) ||

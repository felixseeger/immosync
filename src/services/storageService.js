...
export const uploadPropertyImage = async (file, propertyId, onProgress) => {
    try {
...
export const uploadPropertyVideo = async (file, propertyId, onProgress) => {
    const storageRef = ref(storage, `properties/${propertyId}/${file.name}-${Date.now()}`);
    const uploadTask = uploadBytesResumable(storageRef, file);
    return new Promise((resolve, reject) => {
        uploadTask.on('state_changed', (snapshot) => {
...
export const uploadDealDocument = async (file, dealId, onProgress) => {
